...
function updateSuper(p){
 ...
 if(p.superState==="dash"){p.superT--;p.vx=p.face*24;p.x+=p.vx;if(p.x<70||p.x>W-70||p.superT<=0){p.x=clamp(p.x,70,W-70);p.superState="end";p.superT=38;p.active=false;p.vx=0;p.atk=38;p.inv=16;shake=14;screenFlash=6;msg="SUPER END!";msgT=20;spark(p.x,p.y-90,40,"#ff4444")}return true}
...
function control(p){
 ...
 if(keys[L]){p.vx=-sp;if(!p.atk)p.face=-1}
 if(keys[R]){p.vx=sp;if(!p.atk)p.face=1}
 ...
 if(kickPressed(p)){p.face=enemy.x>=p.x?1:-1;if(startKick(p))return}
