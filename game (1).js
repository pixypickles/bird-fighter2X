...
let state="title",round=1,timer=70,tick=0,shake=0,freeze=0,msg="",msgT=0,bullets=[],parts=[],koFlash=0,screenFlash=0,meterTick=0,pendingKO=false;
let clouds=[];
...
function updateSuper(p){
 ...
 if(p.superState==="dash"){p.superT--;p.vx=p.face*24;if(p.x<70||p.x>W-70||p.superT<=0){p.x=clamp(p.x,70,W-70);p.superState="end";p.superT=38;p.active=false;p.vx=0;p.atk=38;p.inv=16;shake=14;screenFlash=6;msg="SUPER END!";msgT=20;spark(p.x,p.y-90,40,"#ff4444")}return true}
...
function melee(a,t){
 ...
 if(a.type==="super"&&a.superState==="dash"){let box={x:a.face>0?a.x+12:a.x-150,y:a.y-158,w:150,h:96};if(hit(box,rect(t))&&!a.superHit){a.superHit=true;t.guard=false;t.down=true;t.downT=90;t.stun=90;t.hp=clamp(t.hp-28,0,120);t.vx=a.face*28;t.vy=-12;t.face=-a.face;if(t.hp<=0)pendingKO=true;freeze=12;shake=24;screenFlash=10;msg="PHOENIX HIT!";msgT=36;spark(t.x,t.y-95,70,"#ff3131")}return}
