...
function control(p){
 ...
 p.vx=0;let ch=roster[p.charIndex||0]||roster[0];let sp=(p.guard?1.25:4.35)*ch.spd;
 if(keys[L]){p.vx=-sp;if(grounded(p)||!p.atk)p.face=-1}
 if(keys[R]){p.vx=sp;if(grounded(p)||!p.atk)p.face=1}
 if(keys[U]&&grounded(p)&&!p.guard){p.vy=-14.5;if(keys[L])p.vx=-6.2*ch.spd;else if(keys[R])p.vx=6.2*ch.spd;else p.vx*=.4}
